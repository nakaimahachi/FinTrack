...
<style>
    #MainMenu { visibility: hidden; }
    footer { visibility: hidden; }
    /* Keep header at 0px but visible so collapsedControl works */
    header, [data-testid="stHeader"] {
...
def show_app():
    ...
    with st.sidebar:
        ...
        page = st.radio("Navigate", [
            "Dashboard", "Family Summary", "Add Transaction",
            "Budgets", "Manage Categories", "All Transactions"
        ])
        st.markdown("---")
        ...
        selected_year = st.selectbox(
            "Year", list(range(2023, today.year + 1)),
            index=list(range(2023, today.year + 1)).index(today.year)
        )
        st.markdown("---")
    ...
    if page == "Dashboard":
        ...
        try:
            summary = get_summary(user_id=user["id"], period=period, year=selected_year, month=selected_month)
        except Exception:
        ...
        with col1:
            st.markdown(f'<div class="metric-card"><div class="metric-label">Income</div><div class="metric-value income">€{summary["income"]:,.2f}</div></div>', unsafe_allow_html=True)
        with col2:
            st.markdown(f'<div class="metric-card"><div class="metric-label">Expenses</div><div class="metric-value expense">€{summary["expenses"]:,.2f}</div></div>', unsafe_allow_html=True)
        with col3:
            bal_class = "balance-pos" if summary["balance"] >= 0 else "balance-neg"
            st.markdown(f'<div class="metric-card"><div class="metric-label">Balance</div><div class="metric-value {bal_class}">€{summary["balance"]:,.2f}</div></div>', unsafe_allow_html=True)
        ...
        if alerts:
            ...
            for a in alerts:
                ...
                st.markdown(f'<div class="alert-{a["status"]}">{icon} <strong>{a["category"]}</strong> — €{a["spent"]:.2f} / €{a["limit"]:.2f} ({a["pct"]:.0f}%)</div>', unsafe_allow_html=True)
        ...
        if not df.empty:
            ...
            with col_l:
                ...
                if not exp_df.empty:
                    ...
                    fig = px.pie(cat_totals, values="amount", names="category",
                                  color_discrete_sequence=["#f5d782","#c9960c","#8B6914","#4ade80","#f87171","#a78bda","#60a5fa"],
                                  hole=0.45)
            ...
            with col_r:
                ...
                timeline = df.groupby(["date", "type"])["amount"].sum().reset_index()
                fig2 = px.bar(timeline, x="date", y="amount", color="type",
                               color_discrete_map={"income": "#4ade80", "expense": "#f87171"}, barmode="group")
                fig2.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                                    font_color="white", xaxis=dict(gridcolor="#1a0035"),
                                    yaxis=dict(gridcolor="#1a0035"), margin=dict(t=20, b=20))
                st.plotly_chart(fig2, use_container_width=True)

            st.markdown("### Cash vs Card")
            method_df = df.groupby("payment_method")["amount"].sum().reset_index()
            fig3 = px.bar(method_df, x="payment_method", y="amount", color="payment_method",
                           color_discrete_map={"cash": "#f5d782", "card": "#8B6914"})
            fig3.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                                 font_color="white", showlegend=False, margin=dict(t=20, b=20))
            st.plotly_chart(fig3, use_container_width=True)

            st.markdown("### Recent Transactions")
            display = df[["date", "description", "category", "type", "payment_method", "amount"]].head(10).copy()
            display["amount"] = display.apply(
                lambda r: f"+€{r['amount']:.2f}" if r["type"] == "income" else f"-€{r['amount']:.2f}", axis=1)
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown(f'<div class="metric-card"><div class="metric-label">Family Income</div><div class="metric-value income">€{family_summary["income"]:,.2f}</div></div>', unsafe_allow_html=True)
        with col2:
            st.markdown(f'<div class="metric-card"><div class="metric-label">Family Expenses</div><div class="metric-value expense">€{family_summary["expenses"]:,.2f}</div></div>', unsafe_allow_html=True)
        with col3:
            bal = family_summary["balance"]
            bal_class = "balance-pos" if bal >= 0 else "balance-neg"
            st.markdown(f'<div class="metric-card"><div class="metric-label">Family Balance</div><div class="metric-value {bal_class}">€{bal:,.2f}</div></div>', unsafe_allow_html=True)
        ...
        with st.form("add_tx_form", clear_on_submit=True):
            ...
            if submitted:
                ...
                if not description.strip():
                    ...
                    add_transaction(user["id"], amount, description.strip(),
                                     tx_date, payment_method, cat_map[category_name])
        ...
        try:
            df = get_transactions_df(user_id=user["id"], period=period,
