...
def show_app():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Navigate",
            ["Dashboard", "Family Summary", "Add Transaction", "Budgets", "Manage Categories", "All Transactions"]
        )
    ...
    if page == "Dashboard":
        ...
        try:
            df = get_transactions_df(user_id=user["id"], period=period, year=selected_year, month=selected_month)
        except Exception:
            df = pd.DataFrame()
